...
>
    ...
    <div>
      ...
      {#if quickEntryMode}
        <p class="text-sm text-blue-600">
          Quick Entry Mode • Transaction #{transactionCount + 1}
        </p>
      {/if}
    </div>
    ...
    use:enhance={({ formData }) => {
      return async ({ result }) => {
        if (result.type === 'failure') {
          showError((result.data as any)?.error || 'Failed to add transaction');
  ...
  >
    ...
    {#if selectedCategory}
      ...
      >
        <span class="text-lg">{isIncome ? '💰' : isExpense ? '💸' : '💱'}</span>
        <span
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
      ...
      <div>
        <label
          ...
          class="block text-sm font-medium text-gray-700 mb-2">Account</label
        >
        <select
      ...
      <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
        ...
        >
          ...
          <div class="flex-1">
            ...
            >
            <p class="text-xs text-purple-600">Split into multiple payments</p>
          </div>
        </div>
      </div>
    ...
    <div class="flex gap-3 pt-4">
      ...
      <button
        ...
        >{quickEntryMode ? '✅ Add & Continue' : '✅ Add Transaction'}</button
      >
    </div>
  </form>
</Modal>
