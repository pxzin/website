...
describe('calculateProjections', () => {
  ...
  const initialTotalBalance = mockAccounts.reduce(
    (sum, acc) => sum + acc.current_balance,
    0
  );

  it('should calculate projections correctly with no special transactions', () => {
    // Create a transaction for next month to test projection logic
    ...
    expect(projections[0].projected_balance).toBeCloseTo(
      initialTotalBalance + transactions[0].amount
    );
  ...
  it('should handle recurrent transactions correctly', () => {
    ...
    expect(projections[2].projected_balance).toBeCloseTo(
      initialTotalBalance + 2000 * 3
    );
    expect(projections[0].transactions.length).toBe(1);
  ...
  it('should handle installment transactions correctly', () => {
    // Set installments to start next month
    const nextMonth = new Date();
    nextMonth.setMonth(nextMonth.getMonth() + 1);
    ...
    const transactions: Transaction[] = [
      {
        ...
        installments_paid: 0,
        installment_start_date: nextMonth.toISOString().split('T')[0],
      },
    ];
  ...
  it('should handle a mix of recurrent and installment transactions', () => {
    ...
    expect(projections[0].projected_balance).toBeCloseTo(
      initialTotalBalance + 2000 - 1000 - 600 / 6
    );
    expect(projections[0].transactions.length).toBe(3);

    // Month 2: Previous (1400) + Salary (2000) - Rent (1000) - Car (100) = 2300
    expect(projections[1].projected_balance).toBeCloseTo(
      initialTotalBalance + (2000 - 1000 - 600 / 6) * 2
    );
